...
def execute_post_details_fasapi(**kwargs):
    ti = kwargs['ti']
    ...
    try:
        ...
        max_empty_polls = 2
        if response.status_code == 200:
            print("Scraping movie details running successfully")
...
def print_json(**kwargs):
    ti = kwargs['ti']
    json_data = ti.xcom_pull(task_ids='execute_post_details_fasapi')  # Obtener el JSON de la tarea anterior
    if json_data:
